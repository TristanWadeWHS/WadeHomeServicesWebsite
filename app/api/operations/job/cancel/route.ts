import { cancelJob } from "@/app/lib/booking/google";
...
export async function POST(request: Request) {
  ...
  const limit = rateLimit(`operations-cancel:${ip}`, 15, 10 * 60 * 1000);
  if (!limit.ok) return jsonError("Too many cancellation attempts.", 429);
  ...
  try {
    const result = await cancelJob(leadId, reason, authorization.user.label);
    if (!result.ok) return jsonError(result.message, 409, { lead: result.lead });
    return Response.json(result);
  } catch (error) {
    logServerError("operations.cancel", error, { ip, leadId });
    return jsonError("Job cancellation could not be completed safely.", 503);
  }
}
